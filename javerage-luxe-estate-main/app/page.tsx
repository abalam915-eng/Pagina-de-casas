...
import localProperties from "@/lib/data/properties.json";

export const dynamic = 'force-dynamic';

type LocalHomeProperty = {
  id: string;
  title: string;
  location: string;
  price: number;
...
function buildLocalProperty(item: LocalHomeProperty): Property {
  return {
    id: item.id,
    slug: item.id,
    title: item.title,
...
export default async function Home({
  ...
  const localFeatured = localProperties.featured.map(item => ({
    ...buildLocalProperty(item),
    is_featured: true,
  }));

  const localNewInMarket = localProperties.newInMarket.map(item => ({
    ...buildLocalProperty(item),
    is_featured: false,
    category: 'House',
  }));

  const displayedFeatured = featured.length > 0 ? featured : localFeatured;
  const displayedNewInMarket = newInMarket.length > 0 ? newInMarket : localNewInMarket.slice(0, limit);
  const totalResults = newInMarket.length > 0 ? count : localNewInMarket.length;
  ...
  return (
    <div className="min-h-screen bg-clear-day">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
        ...
        {!isFiltered && displayedFeatured.length > 0 && (
        ...
        <section>
          <div className="flex items-end justify-between mb-8">
            <div>
              ...
              <p className="text-nordic/60 mt-1 text-sm">
                {isFiltered 
                  ? tHome('results_found', { count: totalResults }) 
          ...
          {displayedNewInMarket.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {displayedNewInMarket.map((prop) => (
